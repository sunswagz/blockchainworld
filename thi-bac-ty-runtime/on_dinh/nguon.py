"""Nguồn: đỉnh sổ lệnh giao ngay của ba sàn. Dữ liệu CÔNG KHAI, không khoá.

Ba sàn, ba hình dạng JSON khác nhau, một kiểu dữ liệu ra. Đây đúng là chỗ
bản đồ nói tới khi đòi một lớp Data World — nhưng lớp ấy chưa dựng được
thành schema chung, vì hai ty trước đọc hai thế giới không giao nhau. Xem
`thi_bac_ty/nguon.py`.

Cái CÓ dùng chung ngay là kỷ luật: sổ sức khoẻ, lọc NaN/inf ở cửa, tuổi
dữ liệu đọc được từ ngoài.
"""
