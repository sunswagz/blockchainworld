/* ═══════════════════════════════════════════════════════
   Service worker — Tử Cấm Thành
   Chiến lược:
     · vỏ ứng dụng (html/css/js/icon) : cache trước, chạy offline
     · phông chữ Google              : cache khi dùng lần đầu

   `assets/js/v/phien.js` KHÔNG nằm trong SHELL, và đó là chủ ý.

   Nó là lát cắt phiên giao dịch do runtime ghi ra rồi commit. Để nó ở
   nhánh cache-trước thì máy đã cài app sẽ hiện phiên của lần trước cho
   tới lần nâng CACHE_VERSION kế tiếp — tức là một bảng số liệu nói dối,
   thứ tệ hơn hẳn không có bảng nào. Đúng cái bẫy đã cắn `logos.js` của
   Công Bộ và `van-hanh.js` của Tạo Biện Xứ. Nên nó đi nhánh MẠNG-TRƯỚC.

   Đổi CACHE_VERSION mỗi lần phát hành để đẩy bản mới xuống máy. Quên
   nâng thì máy đã cài cứ dùng bản cũ — xem mục "Sửa file trong SHELL
